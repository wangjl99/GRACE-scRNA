...
Input:   results/hcc/hcc_baseline_results.json   (from Day 2)
Outputs:
  results/hcc/hcc_versionB_results.json          ← grounded narratives + confidence scores
  results/hcc/hcc_comparison_table.csv           ← all 3 methods side-by-side
...
BASELINE_JSON = RESULTS_DIR / "hcc/hcc_baseline_results.json"
...
def orchestrate(
    ...
    # Updated 4-agent weighted formula
    # Pathway agent keeps highest weight (most reliable: 18/20 confirmed)
    # Cell identity agent replaces half of DEG validator weight
    # Agent 5 — Regulatory
    import pandas as _pd
    from pathlib import Path as _pathlib_Path
    _deg_path = _pathlib_Path(f"results/hcc/degs/cluster_{cluster_id}_degs.csv")
    if _deg_path.exists():
...
def build_comparison_table(results: list[dict]) -> pd.DataFrame:
    ...
    df.to_csv(RESULTS_DIR / "hcc/hcc_comparison_table.csv", index=False)
    print(f"  Saved → results/hcc/hcc_comparison_table.csv")
    return df
...
def main():
    ...
    for cl_id in tqdm(sorted(clusters.keys(), key=int), desc="Clusters"):
        ...
        try:
            import pandas as _pd
            _df = _pd.read_csv(
                Path("results/hcc/degs") / f"cluster_{cl_id}_degs.csv")
    ...
    # Save full results
    out_json = RESULTS_DIR / "hcc/hcc_versionB_results.json"
    ...
    print(f"  results/hcc/hcc_versionB_results.json")
    print(f"  results/hcc/hcc_comparison_table.csv  ← Table 1 draft")
